...
| [`case-study-self-pii-recall-2026-07.md`](./case-study-self-pii-recall-2026-07.md) | July 2026 | Post-mortem on the recalled credential-audit case study. Walks the timeline (audit → writeup → publish → operator catch → recall), names the structural failures (no pre-publish gate, audit-as-evidence confusion, no friction on publish), and shows the rule that comes out of it. The same repo that already hosted the `self-pii-audit-2026-07` writeup against shipping PII was the repo that shipped PII — fail mode was not a missing rule but a missing workflow. |
